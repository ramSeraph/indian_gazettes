...
img_data_dir = f'{script_dir}/data/imgs/'
model_dir = f'{script_dir}/data/models/'
...
def guess(filename, model_dir=model_dir, manual=False):
    if manual:
...
if __name__ == '__main__':
    ...
    imgs_dir = f'{script_dir}/data/imgs/'
    filenames = glob.glob(imgs_dir + '*.jpeg')
    filenames = [ f'{script_dir}/data/imgs/1.jpeg' ]
    for filename in filenames:
        logger.info(f'handling {filename}')
        guessed = guess(filename, model_dir)
        print(guessed)
